<template>
  <div class="d-flex flex-column justify-center align-center overflow-y-auto">
    <v-card
      ...
      <v-dialog v-model="addModelDialog" width="500">
        <template #activator="{ on, attrs }">
          <v-btn
            ...
            v-on="on"
          >
            <v-icon>mdi-plus</v-icon>新建</v-btn
          >
        </template>
        <v-card>
    ...
    >
      <template #item.actions="{ item }">
        <v-btn @click="enterViewModel(item, 'ETL')">ETL编辑</v-btn>
        <v-btn @click="publishETL(item)">发布</v-btn>
        <v-btn>导出</v-btn>
        <v-btn>编辑</v-btn>
        <v-btn @click="delEtl(item)">删除</v-btn>
      </template>
    </v-data-table>
  </div>
</template>
